<script>
  ...
  let weatherGrid = (component) => {
    return gridHelp.item({ x: 0, y: 0, w: 10, h: 1, id: id(), component: component })
  } 
  ...
  function handleWeatherClick(item, event) {
    if (!isWeatherVisible) {
      ...
      return gridHelp.item({ x: 0, y: 0, w: 3, h: 1, id: id(), component: WeatherGrid });
    } else {
      isWeatherVisible = !isWeatherVisible
      let newItem = gridHelp.item({ x: 0, y: 0, w: 4, h: 7, id: id(), component: WeatherGrid  });
      remove(item, event)
...
<Grid bind:items let:item cols={10} rowHeight={100} gap={10}>
  <div class="content" transition:fade="{{delay: 250, duration: 300}}">
    <span on:click={remove.bind(null, item)} class=close>x</span>
    <!-- <WeatherGrid visible={isWeatherVisible} /> -->
    <svelte:component this={item.component} visible={isWeatherVisible} on:toggle={handleWeatherClick} />
  </div>
</Grid>
